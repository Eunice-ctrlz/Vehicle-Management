    ...
    if request.method == "POST":
        ...
        try:
            ...
            if pickup_date < timezone.now():
                ...
                days = duration.days
                if duration.seconds > 3600: # If there's more than an hour extra, charge a full day
                    days += 1
                days = max(1, days)
                ...
                # Optional: Only mark unavailable if your business logic requires 
                # immediate locking. Otherwise, do this in the 'Approve' view.
...
def my_bookings(request):
    """
    Categorizes bookings into Active, Upcoming, and Past for a cleaner UI.
    """
    now = timezone.now()
    ...
    active_bookings = all_user_bookings.filter(
        status='confirmed', # Or 'active' if you have that status
...
def cancel_booking(request, booking_id):
    """
    Allows users to cancel only if the booking is still pending or early-stage.
    """
